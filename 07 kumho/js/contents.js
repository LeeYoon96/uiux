...
$(document).ready(function(){
    ...
    function his_head(){
        ...
        let obj_top // 위에서부터 해당 요소 위까지의 거리
        ...
        for(i=0; i<obj_length; i++){
            ...
            // console.log(i, scrolling, obj_end)
    ...
    function his_area(){
        ...
        let obj_start // 각 애니메이션 시작 위치
        let obj_end // 애니메이션 종료 위치
        let scrolling = $(window).scrollTop() // 스크롤 값
        let wid_h = $(window).height() // 브라우저 높이
        //console.log(obj_length)

        for(i=0; i<obj_length; i++){
            obj_top = obj_name.eq(i).offset().top
            obj_start = (obj_top -wid_h) + (wid_h * 0.5)
            obj_end = obj_top + obj_name.eq(i).height() - (wid_h * 0.5)
